...
def regnetx(name, pretrained=False):
    ...
    model = AnyNet(**{
        ...
        "ss": [2, 2, 2, 2], "bms": [1.0, 1.0, 1.0, 1.0], "se_r": None, "nc": 1000,
...
def resnet(name, pretrained=False, nc=1000):
    ...
    model = AnyNet(**{
        "stem_type": "res_stem_in", "stem_w": 64, "block_type": "res_bottleneck_block",
        "ss": [1, 2, 2, 2], "bms": [0.25, 0.25, 0.25, 0.25], "se_r": None, "nc": nc,
        "ds": cfg["ds"], "ws": [256, 512, 1024, 2048], "gws": [64, 128, 256, 512]
    })
    # Download and load the weights
    if pretrained:
...
def resnext(name, pretrained=False, nc=1000):
    ...
    model = AnyNet(**{
        "stem_type": "res_stem_in", "stem_w": 64, "block_type": "res_bottleneck_block",
        "ss": [1, 2, 2, 2], "bms": [0.5, 0.5, 0.5, 0.5], "se_r": None, "nc": nc,
        "ds": cfg["ds"], "ws": [256, 512, 1024, 2048], "gws": [4, 8, 16, 32]
    })
    # Download and load the weights
    if pretrained:
...
# EN -> cfg
_EN_CFGS = {
    "B0": {"sw": 32, "ds": [1, 2, 2, 3, 3, 4, 1], "ws": [16, 24, 40, 80, 112, 192, 320], "hw": 1280},
    "B1": {"sw": 32, "ds": [2, 3, 3, 4, 4, 5, 2], "ws": [16, 24, 40, 80, 112, 192, 320], "hw": 1280},
    "B2": {"sw": 32, "ds": [2, 3, 3, 4, 4, 5, 2], "ws": [16, 24, 48, 88, 120, 208, 352], "hw": 1408},
    "B3": {"sw": 40, "ds": [2, 3, 3, 5, 5, 6, 2], "ws": [24, 32, 48, 96, 136, 232, 384], "hw": 1536},
    "B4": {"sw": 48, "ds": [2, 4, 4, 6, 6, 8, 2], "ws": [24, 32, 56, 112, 160, 272, 448], "hw": 1792},
    "B5": {"sw": 48, "ds": [3, 5, 5, 7, 7, 9, 3], "ws": [24, 40, 64, 128, 176, 304, 512], "hw": 2048},
}
...
def effnet(name, pretrained=False, nc=1000):
    ...
    cfg = _EN_CFGS[name]
    model = EffNet(**{
        "exp_rs": [1, 6, 6, 6, 6, 6, 6], "se_r": 0.25, "nc": nc,
        "ss": [1, 2, 2, 2, 1, 2, 1], "ks": [3, 3, 5, 3, 5, 5, 3],
        "stem_w": cfg["sw"], "ds": cfg["ds"], "ws": cfg["ws"], "head_w": cfg["hw"]
    })
    # Download and load the weights
    if pretrained:
